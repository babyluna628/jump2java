...
public class Ch03_리스트 {
    public static void main(String[] args) {
        ...
        String result = String.join("요일, ", weeks);
        System.out.println(result);

        //리스트 정렬하기
        n.sort(Comparator.reverseOrder()); //역순정렬
        System.out.println(n); //3,2,1 로 역순정렬됨
        //정방향으로 정렬
        weeks.sort(Comparator.naturalOrder());//정방향(오름차순)
        System.out.println(weeks); //한글은 가나다라마바사 영어는abcdef
